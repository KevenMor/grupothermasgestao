...
-- Execute este script para corrigir a estrutura da tabela
-- =====================================================
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Tabela usuarios já existe - verificando estrutura...';
        
        -- Verificar se a coluna cargo existe
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'usuarios' AND column_name = 'cargo') THEN
            ALTER TABLE usuarios ADD COLUMN cargo VARCHAR(100) NOT NULL DEFAULT 'usuario';
...
-- Mostrar estrutura atual da tabela usuarios
SELECT 
    column_name,
    data_type,
    is_nullable,
